...
if ($_SERVER["REQUEST_METHOD"] == "POST" && isset($_POST['acao_salvar_reserva'])) {
    ...
    $cod_usuario = !empty($_POST['cod_usuario']) ? (int)$_POST['cod_usuario'] : null; 

    $hoje = date('Y-m-d');

    if (empty($nome) || strlen($cpf) !== 11 || empty($data_checkin) || empty($data_checkout) || empty($cod_unid) || empty($cod_quart) || empty($cod_usuario)) {
        $mensagem = "Por favor, preencha todos os campos obrigatórios corretamente, incluindo a seleção do funcionário responsável.";
...
$usuarios_sistema = $conexao->query("SELECT id, nome FROM usuarios ORDER BY nome ASC")->fetchAll(PDO::FETCH_ASSOC);
...
<div class="container mt-5">
    <div class="mb-4">
        ...
        <h1 class="h2 text-dark">Lançamento de Reserva (Painel de Controle Admin)</h1>
    </div>
        ...
        <form method="POST" id="formReserva">
            ...
            <div class="card-body p-4 bg-light">
                <div class="row g-3">
                    ...
                    <div class="col-md-4">
                        <label class="form-label fw-bold text-dark">Funcionário Responsável (cod_usuario):</label>
                        <select name="cod_usuario" class="form-select border-primary" required>
                            <option value="">-- Selecione o Usuário Destinatário --</option>
                            <?php foreach ($usuarios_sistema as $us): ?>
                                <option value="<?php echo $us['id']; ?>" <?php echo ($us['id'] == 1) ? 'selected' : ''; ?>>
                                    <?php echo htmlspecialchars($us['nome']); ?> (ID: <?php echo $us['id']; ?>)
                                </option>
                            <?php endforeach; ?>
                        </select>
                    </div>
